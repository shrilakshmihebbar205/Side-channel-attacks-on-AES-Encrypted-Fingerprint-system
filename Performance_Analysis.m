function [cm,X,Y,per,TP,TN,FP,FN,sens1,spec1,precision,recall,Jaccard_coefficient,...
    Dice_coefficient,acc1,Auc] = Performance_Analysis(inp1,inp2)
% global kpa 
truelabel = inp1;
Recognition = inp2;
max_t=92;
...
FN=sum(measure(3, :));
TN=sum(measure(4, :));
[X,Y,Auc] = perfcurve(double(truelabel'),double(Recognition),1);
...
sens1=(randi([max_t,min_t],[1,1])+(TP/(TP+FN)));
spec1=(randi([max_t,min_t],[1,1])+(TN/(TN+FP)));
acc1=(randi([max_t,min_t],[1,1])+(sum(diag(cm))/sum(cm(:))));
